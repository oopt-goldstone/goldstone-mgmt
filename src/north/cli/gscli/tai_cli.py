...
class NetIf_CLI (Object):
    ...
    def __init__ (self, transponder, conn, netif_id):
        ...
        self.mod_format = ['bpsk','dp-bpsk','qpsk','dp-qpsk','8-qam','dp-8-qam','16-qam','dp-16-qam','32-qam','dp-32-qam','64-qam','dp-64-qam']
        self.command_list = ['tx-dis','modulation-format','output-power','tx-laser-freq','voa-rx','differential-encoding']

        @self.command (name = "output-power")
        def output_power (args):
            if len (args) != 1:
                raise InvalidInput ('usage: output-power <value_in_db>')
            self.tai_netif.set_output_power (self.transponder_name, self.netif_id, args[0])
        ...
        def modulation_format (args):
            ...
            self.tai_netif.set_modulation_format (self.transponder_name, self.netif_id, args[0])
        ...
        def tx_laser_freq (args):
            ...
            if (len (args[0]) < 14):
                raise InvalidInput ('usage: Enter value between 191150000000000 Hz and 196100000000000 Hz' )
            self.tai_netif.set_tx_laser_freq (self.transponder_name, self.netif_id, args[0])
        ...
        def differential_encoding (args):
            ...
            self.tai_netif.set_differential_encoding (self.transponder_name, self.netif_id, args[0])
        ...
        def no (args):
            ...
            if args[0] in self.command_list :
                self.tai_netif.set_no_command (self.transponder_name, self.netif_id, args[0])
            else: 
                self.no_usage() 
...
class Transponder_CLI (Object):
    def __init__ (self, conn, parent, transponder_name):
        ...
        @self.command (WordCompleter(lambda : self.tai_transponder._components (self.transponder_name, 'network-interface')))
        def netif (args):
            if len (args) != 1:
                raise InvalidInput ('usage: netif <name>')
            return NetIf_CLI (self, conn, args[0])

        @self.command (WordCompleter(lambda : self.tai_transponder._components (self.transponder_name, 'host-interface')))
        def hostif (args):
            if len (args) != 1:
