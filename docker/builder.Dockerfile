...
RUN --mount=type=cache,target=/var/cache/apt --mount=type=cache,target=/var/lib/apt \
            apt update && DEBIAN_FRONTEND=noninteractive apt install -qy libgrpc++-dev g++ protobuf-compiler-grpc make pkg-config python3 curl python3-distutils python3-pip libclang1-6.0 doxygen libi2c-dev git python3-dev cmake swig libpcre3-dev bison graphviz libcmocka-dev valgrind quilt libcurl4-gnutls-dev
...
RUN --mount=type=bind,source=sm/sysrepo,target=/root/sm/sysrepo,rw --mount=type=bind,source=patches,target=/root/patches cd /root && ls && ( /usr/bin/quilt push -af || true ) && mkdir -p /build/sysrepo && cd /build/sysrepo && \
