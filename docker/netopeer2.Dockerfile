...
RUN --mount=type=cache,target=/var/cache/apt --mount=type=cache,target=/var/lib/apt \
            apt update && DEBIAN_FRONTEND=noninteractive apt install -qy make pkg-config curl git cmake libssh-4 libssh-dev libpcre3-dev
...
RUN --mount=type=bind,source=sm/sysrepo,target=/root/sm/sysrepo,rw cd /root && mkdir -p /build/sysrepo && cd /build/sysrepo && \
...
RUN --mount=type=bind,source=sm/netopeer2,target=/root/sm/netopeer2,rw cd /root && mkdir -p /build/netopeer2 && cd /build/netopeer2 && \
